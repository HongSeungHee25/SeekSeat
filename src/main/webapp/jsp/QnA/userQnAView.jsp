...
<!-- 회원 1:1 문의 확인 화면 -->
   ...
   <section id="QnAUser">
      <h2>1:1문의</h2>
      <form action="QnAList?type=1" method="post">
         <table>
            <tr>
               <th>No</th>
               <th>문의자</th>
            ...
            <c:forEach var="vo" items="${list }">
               <tr>
                  ...
                  <td><a href="read?type=1&writeNo=${vo.writeNo }&page=${paging.currentPage}#QnAUser" ><c:out value="${vo.title }" /></a></td>
                  <td>
         ...
         <div
            ...
            <br>
            <hr>
            <a class="pagenum" href="?type=1&page=1#QnAUser">&lt;&lt;</a>
            <!--(1) 첫번째 페이지 1번으로 이동 -->

            <!--(2)  실행하면서 파악해보세요. -->
            <!-- 요청은 ListController가 받음.page파라미터 변경됨. -->
            <a class="pagenum" href="?type=1&page=${paging.startPage-1 }#QnAUser"
               style='<c:if test="${paging.startPage==1 }">display:none;</c:if>'>&lt;</a>

            <!--(3) 페이지 범위 startPage 부터 endPage 까지 반복 -->
            <c:forEach var="i" begin="${paging.startPage }"
               end="${paging.endPage }">
               <a class="pagenum ieach" href="?type=1&page=${i }#QnAUser"><c:out
                     value="${i }" /></a>
            </c:forEach>

            <!--(4)  실행하면서 파악해보세요. -->
            <a class="pagenum" href="?type=1&page=${paging.endPage+1 }#QnAUser"
               style='<c:if test="${paging.endPage==paging.totalPage }">display:none;</c:if>'>&gt;</a>

            <a class="pagenum" href="?type=1&page=${paging.totalPage }#QnAUser">&gt;&gt;</a>
            <!--(5) 가장 마지막 페이지로 이동 -->
         </div>
      </form>
         
      <div class="button">
         <button onclick="location.href='write?type=1'">문의글 작성</button>
      </div>
   </section>
